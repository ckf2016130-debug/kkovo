...
'''
    news_link_ui = r'''document.querySelectorAll('#topNewsList .news-brief a[href="#"]').forEach(a=>{const span=document.createElement('span');span.textContent=a.textContent;a.replaceWith(span)});const newsLinkStyle=document.createElement('style');newsLinkStyle.textContent='#topNewsList .news-brief a{color:var(--text);text-decoration:none}#topNewsList .news-brief a:hover{color:var(--gold);text-decoration:underline}#topNewsList .news-brief b>span{cursor:pointer}';document.head.appendChild(newsLinkStyle)
'''
    trade_plan_ui = stock_agent_ui + valuation_ui + etf_share_ui + news_ui + news_switch_ui + news_detail_ui + overseas_corr_ui + stock_final_ui + news_link_ui + r'''
