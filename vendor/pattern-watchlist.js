(() => {
  ...
  function render(){
    ...
    $('pvDetail').innerHTML=`<h3>${esc(r.name)} <small>${esc(r.ts_code)}</small></h3><p>${esc(r.reason)}</p><div class="pv-facts"><div><small>首段上冲</small>${date(r.event_date)}</div><div><small>阶段变更</small>${date(r.status_date)}</div><div><small>回撤幅度</small>${num(r.depth)}%</div><div><small>后3日量 / 前3日量</small>${num(r.contraction)} 倍</div><div><small>形态失效位（${payload.price_basis==='qfq'?'前复权':'未复权'}）</small>${num(r.invalidation)} 元</div><div><small>个股数据截止</small>${date(r.observed_date)}</div></div><p class="pv-muted">失效位依据首段高点×75%计算，是研究阈值，不是止损成交保证。回撤与量比为最近一次整理判定值，确认后冻结。${payload.price_basis==='qfq'?'除权后历史价格及阈值会随复权重算。':''}</p>${r.stale?'<p class="pv-alert">该股数据落后于全池截止日，不视为最新信号。</p>':''}<p class="pv-alert">${r.single_price?'该记录最近观测日为单一成交价，不能假设能够买入。':'尚未核验次日涨跌停及可成交性。'}</p><button class="btn" id="pvOpenStock">查看个股 K 线与关注操作</button>`;
