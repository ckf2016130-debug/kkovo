...
def parse_quote(values, sym):
    ...
    row = dict(ts_code=canonical(sym), name=values[1], trade_date=values[30][:8],
               ...
               vol=number(values[6]), amount_yi=(number(values[37]) or 0) / 10000,
...
def history(sym, cutoff):
    ...
    body = json.loads(request('https://web.ifzq.gtimg.cn/appstock/app/fqkline/get?param=' + params))
...
def main():
    ...
    def one(code):
        ...
        if old.get('checked_session') == cutoff and old.get('rows'):
